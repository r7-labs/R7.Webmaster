...
namespace R7.Webmaster.Addins.TextCleaner
{
	...
	{
		private List<ITextCleanerCommand> DefaultProcessing;

		public TextCleanerModel ()
		{
			InitDefaultProcessing ();
		}

		public string TextClean (string text, TextCleanParams textCleanParams)
		{
			Params = textCleanParams;

			foreach (ITextCleanerCommand command in DefaultProcessing)
				text = command.Execute (text);

			return text;
		}

		protected TextCleanParams Params;

		private void InitDefaultProcessing ()
		{
			DefaultProcessing = new List<ITextCleanerCommand> () {
		...
		public string TextCleanOld (string text, bool htmlin, bool htmlout, bool fixRssDate, bool clearTables, bool emNames, TextCleanParams param)
		{
